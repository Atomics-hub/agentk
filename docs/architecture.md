...
`agentk release-audit` packages the local release ritual into one report. It runs readiness, git hygiene checks, formatting, tests, clippy, a fresh demo trace, signature verification, signer-pinning and trusted-signer manifest smoke coverage, brokered secret-handle, secret-reference validation, and secret-store availability smoke tests, MCP taint-flow, subprocess MCP boundary, lifecycle-redaction, initialize-guard, tool-shape guard, bad-response redaction, response-timeout, transport-close, mixed-interop, notification-burst/flood, no-passthrough, and config-guard smoke tests, redacted inspect, replay, fork replay, behavior fork replay, and an MCP server smoke test. It does not configure remotes or push.
