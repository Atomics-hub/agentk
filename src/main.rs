...
fn run_release_ticket(options: ReleaseTicketOptions) -> Result<ReleaseTicketReport, AgentKError> {
    ...
    let github_release_draft = out.join("github-release-draft.md");
    ...
    write_release_ticket_github_release_draft(
    ...
    let github_release_draft_check =
        release_ticket_github_release_draft_check(&github_release_draft, &publication_report)?;
    ...
    let mut artifacts = vec![
        ...
        release_ticket_artifact("GitHub release draft", &github_release_draft)?,
    ];
...
fn write_release_ticket_github_release_draft(
    path: &Path,
    finalization: &ReleaseFinalizeReport,
    publication: &ReleasePublicationCheckReport,
) -> Result<(), AgentKError> {
    let tag = publication.tag.as_deref().unwrap_or("<signed tag pending>");
    let mut out = String::new();
    out.push_str(&format!(
        "# GitHub Release Draft: {}\n\n",
        publication.release
    ));
    out.push_str("This is an offline maintainer draft generated by `release-ticket`. It does not tag, push, upload, or publish.\n\n");
    ...
    out.push_str(&format!(
        "- Finalization report: `{}`\n",
        publication.finalization.display()
    ));
    out.push_str(&format!(
        "- Release notes: `{}`\n",
        publication.notes.display()
    ));
    out.push_str(&format!(
        "- Package archive SHA-256: `{}`\n\n",
        publication.package_archive_sha256
    ));

    out.push_str("## Release Body Source\n\n");
    out.push_str(&format!(
        "Use `{}` as the GitHub release body after final placeholders are replaced and `release-publication-check` passes.\n\n",
        publication.notes.display()
    ));

    out.push_str("## Assets To Attach\n\n");
    for asset in &publication.release_assets {
        let sha = asset.sha256.as_deref().unwrap_or("<missing>");
        out.push_str(&format!(
            "- `{}`: `{}` (sha256 `{}`)\n",
            asset.name,
            asset.path.display(),
    ...
    out.push_str("\n## Publication Steps\n\n");
    for step in &publication.publication_steps {
        out.push_str(&format!(
            "- `{}`: {} Evidence: `{}` Gate: {}\n",
            step.owner, step.action, step.evidence, step.publish_gate
        ));
    ...
    out.push_str("\n## Final Gate\n\n");
    out.push_str("Publish only after the signed tag is verified, the strict finalization report is ready, release notes match final evidence, and `release-publication-check` passes.\n");
    if !finalization.ready {
        out.push_str("\nWarning: this ticket finalization report is not ready yet.\n");
    }
    Ok(fs::write(path, out)?)
}

fn release_ticket_github_release_draft_check(
    path: &Path,
    publication: &ReleasePublicationCheckReport,
    ...
    for fragment in [
        "offline maintainer draft",
        "It does not tag, push, upload, or publish.",
        &publication.release,
        tag,
        "Assets To Attach",
        "Publication Steps",
    ...
    Ok(release_ticket_check_item(
        ...
        if missing.is_empty() {
            "github-release-draft.md records release body source, signed tag, asset hashes, owner steps, and no-publish scope"
                .to_string()
        } else {
            format!(
                "github-release-draft.md is missing required fragments: {}",
                missing.join(", ")
            )
        },
    ))
...
"#
    ...
    fn release_publication_check_accepts_matching_strict_finalization_and_notes() {
        ...
        let draft = test_temp_path("agentk-github-release-draft", "md");
        write_release_ticket_github_release_draft(&draft, &finalized, &publication)
            .expect("GitHub release draft should write");
        let draft_content = fs::read_to_string(&draft).expect("draft should read");
        assert!(draft_content.contains("GitHub Release Draft: v0.2-alpha"));
        assert!(draft_content.contains("It does not tag, push, upload, or publish."));
        assert!(draft_content.contains("v0.2.0-alpha.2"));
        assert!(draft_content.contains("Assets To Attach"));
        assert!(draft_content.contains("sidecar package archive"));
        assert!(draft_content.contains(&report.package_archive_sha256));
        assert!(draft_content.contains("Publication Steps"));
        assert!(draft_content.contains("GitHub release owner"));
        let draft_check = release_ticket_github_release_draft_check(&draft, &publication)
            .expect("draft check should run");
        assert_eq!(draft_check.status, ReadinessStatus::Pass);
